...
class Unit:
    ...
    def tests(self: Unit) -> Iterable[str]:
        cpp_requirements = [requirement for requirement in self.requirements if requirement.endswith(".cpp")]
        cpp_dependencies = [dependency for dependency in self.dependencies if dependency.endswith(".cpp")]
...
def generate_build_autograder_zip(units: Iterable[Unit]) -> str:
    ...
    buf.extend(
        ["zip -r autograder.zip .", "rm -rf ./submission/"]
    )
...
def main() -> None:
    ...
    assert lab_path.exists() and autograder_path.exists() and spec_path.exists() and handout_path.exists()
