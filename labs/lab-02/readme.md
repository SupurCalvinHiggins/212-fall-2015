...
1. If the target value is not in the input vector, the algorithm will check all pairs of indices $i, j$
   ...
   is $$T(n) = \sum\limits_{i=0}^{n-1} \sum\limits_{j=0}^{i - 1} 2 = 2 \sum\limits_{i=0}^{n-1} i = n(n-1)$$
...
1. If the target value is not in the input vector, the algorithm will check all triples of indices $i, j, k$
   ...
   read $A[i]$
   one to read $A[j]$, and one to read $A[k]$. Therefore, the total number of array access
   is $$T(n) = \sum\limits_{i=0}^{n-1} \sum\limits_{j=0}^{i - 1} \sum\limits_{k=0}^{j - 1} 3 = 3 \sum\limits_{i=0}^{n-1} \sum\limits_{j=0}^{i - 1} j = 3 \sum\limits_{i=0}^{n-1} \frac{i(i-1)}{2} = \frac{3}{2} \left( \sum\limits_{i=0}^{n-1} i^2 - \sum\limits_{i=0}^{n-1} i \right) = \frac{3}{2} \left( \frac{n(n-1)(2n - 1)}{6} - \frac{n(n-1)}{2} \right) = \frac{n(n-1)(n-2)}{2}$$
   Depending on your
   implementation, the
   correct formula might vary.
